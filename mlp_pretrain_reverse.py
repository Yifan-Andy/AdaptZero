# Pre-train with reverse X and ADJ
...
from utils import parse_args, transform_coo_to_csr, transform_sp_csr_to_coo, build_knn_sparse_adj, build_knn_sparse_adj_batch, deepwalk_from_edges
...
if __name__ == "__main__":
    ...
    feature_path = f"checkpoints/processed_features/{args.dataset}_reverse.pt"
    if not os.path.exists(feature_path):
        # get knn edges from features
        if features.shape[0] < 1000000:
            adj = build_knn_sparse_adj(features)
        else:
            adj = build_knn_sparse_adj_batch(features, args.device)

        # get deepwalk features from adj
        adj_c = adj.coalesce()
    ...
    np.save(args.embedding_path + args.dataset + '_mlp_reverse.npy', node_embedding)
    ...
    #     file.write(f'REVERSE Args: {args}\n')
    #     file.write('----------------------------------------------\n')
